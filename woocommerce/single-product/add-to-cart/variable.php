...
/** 1) Variaciones filtradas con stock > 0 */
$filtered_variations = array_filter($available_variations, function ($variation) {
    ...
    if (preg_match('/(\d+)/', $variation['availability_html'], $m)) {
        return intval($m[1]) > 0;
    }
    return false;
});
...
/** 2) Cantidades actuales en carrito por ID de variación */
$cart_quantities = [];
if (WC()->cart) {
...
/** 4) Mapa color (slug=>hex) a partir de las variaciones disponibles */
...
foreach (array_keys($variation_color_slugs) as $slug) {
    ...
    if ($term && !is_wp_error($term)) {
        ...
        $color_map[$slug] = $hex ?: '#cccccc';
    } else {
        $color_map[$slug] = '#cccccc';
    }
}

/** (Opcional) Comentario HTML para depurar qué se envía */
...
<form
    ...
    x-data="Object.assign(
        alpineCart(),
        alpineColors(<?php echo wp_json_encode($color_map, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>)
    )"
    ...
    data-cart_quantities='<?php echo json_encode($cart_quantities, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>'
>
    ...
    <div class="mb-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            ?>
                <button
                    type="button"
                    @click="selected_pa_talla = '<?php echo esc_js($slug_value); ?>'; selected_pa_color = ''; errorMessage = ''; updateMaxQty()"
                    :class="selected_pa_talla === '<?php echo esc_js($slug_value); ?>'
    ...
    <div class="mb-4" x-show="selected_pa_talla" x-transition>
        ...
        <div class="flex flex-wrap gap-2" x-init="console.log('colorMap:', colorMap)">
...
<script>
/**
 * Helpers de color para Alpine (se fusionan con alpineCart())
 * Requiere que el layout tenga @stack('scripts') antes de </body>.
 */
