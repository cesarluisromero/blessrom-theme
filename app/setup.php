...
/** -----------------------------------------------------------------
 * 1) No calcular ni mostrar envío en el CARRITO.
 * ----------------------------------------------------------------*/
...
/** Oculta por completo la fila de envío en el CARRITO */
add_filter('woocommerce_cart_needs_shipping', function($needs){
    if (is_cart()) return false;
    return $needs;
}, 10, 1);

/** Vacía el HTML de la fila de envío en CARRITO (por si el tema la imprime igual) */
add_filter('woocommerce_cart_totals_shipping_html', function($html){
    if (is_cart()) return ''; // nada de “¡GRATIS!” aquí
    return $html;
}, 10, 1);

/** Oculta mensajes “no hay opciones de envío” en CARRITO */
...
/** -----------------------------------------------------------------
 * 2) En CHECKOUT: ocultar envío hasta que el cliente elija ciudad/distrito.
 *    (solo mostrar cuando shipping_city tenga valor)
 * ----------------------------------------------------------------*/
add_filter('woocommerce_cart_totals_shipping_html', function($html){
    if (is_checkout()) {
        $city = '';
        if (WC()->customer) $city = WC()->customer->get_shipping_city();
        if (empty($city)) return ''; // sin ciudad => no mostrar nada
    }
    return $html;
}, 20, 1);

add_filter('woocommerce_package_rates', function($rates, $package){
    // si no hay ciudad aún en checkout, no toques ni muestres tarifas
    if (is_checkout()) {
        $dest = isset($package['destination']) ? $package['destination'] : [];
        $city = $dest['city'] ?? (WC()->customer ? WC()->customer->get_shipping_city() : '');
        if (empty($city)) return []; // oculta métodos hasta que elija ciudad
    }
    return $rates;
}, 20, 2);

/** También ocultar mensajes “no hay envío disponible” en CHECKOUT si aún no hay ciudad */
add_filter('woocommerce_no_shipping_available_html', function($html){
    if (is_checkout()) {
...
/** -----------------------------------------------------------------
 * 3) Refrescar el checkout cuando el cliente cambie la ciudad/distrito.
 * ----------------------------------------------------------------*/
